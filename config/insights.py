...
""",
    ...
    # ── WORLD: REGIME, EMERGING MARKETS ─────────────────────────────────────

    "macro_world_regime": """\
**How to read this chart:** Each dot places an economy by two three-month changes: its OECD leading indicator on the horizontal axis (right = growth momentum improving) and its CPI inflation rate on the vertical axis (up = inflation rising). The quadrant names are shorthand for the combination: Goldilocks (growth up, inflation down), Overheating (both up), Stagflation (growth down, inflation up) and Slowdown (both down). The faint dot and arrow show where the same reading stood a month earlier. Economies without enough CPI history are left off and named in the note.

**Practical takeaway:** The chart shows momentum, not levels: an economy with 5% inflation that is easing sits below the axis. Moves across quadrants matter more than position within one; a shift from Goldilocks towards Overheating is the pattern that tends to precede tighter policy. For India the useful comparisons are with the US (Fed direction, dollar and portfolio flows) and China (import prices and competition in third markets). The euro area's growth reading uses OECD's four-largest-economies aggregate, and the two readings for one economy can refer to different months when one statistic is released later (UK CPI, for example, lags the leading indicator by a month).
""",
...
""",
    ...
    "btc_gold_ratio": """\
**How to read this chart:** How many ounces of gold one bitcoin buys, weekly since 2015, on a log scale so that a doubling looks the same wherever it happens. Both assets are held for the same reason — a store of value outside the banking system — but they answer to different conditions: gold to real rates, the dollar and geopolitics; bitcoin to risk appetite and liquidity. The ratio strips the dollar out of both and asks only which one is winning. The dashed line marks parity, one bitcoin for one ounce, crossed for good in 2017. The dots mark the turns of each cycle, labelled with the ratio at that point. Bitcoin's price comes from Coin Metrics, gold from the futures contract.

**Practical takeaway:** The long climb is the whole argument for bitcoin as digital gold: a quarter of an ounce in early 2015, past parity in 2017, and 35 ounces by late 2021. What has happened since is the part worth watching. The October 2021 high of 35 and the January 2025 high of 38 are barely distinguishable, so through four years of ETF approval, a halving and a corporate treasury bid, bitcoin gained almost nothing on gold. The ratio now sits near 18, about half its peak, because gold has risen around 20% over the past year while bitcoin has fallen about 30%. Drawdowns against gold are also far deeper than the gold price alone suggests: the ratio fell by more than 80% in 2018 and by three quarters in 2022. Treating the two as interchangeable hedges understates how much more violently the newer one moves.
""",
...
""",
    ...
    "btc_mvrv": """\
**How to read this chart:** The top panel is bitcoin's price on a log scale. The bottom panel is MVRV: bitcoin's market value divided by its realised value, which prices every coin at the price it last moved on-chain and so approximates what holders paid in total. At 1, the market price equals that cost basis; below 1 (the green band), the average holder is at a loss; far above it, holders sit on large paper gains. The dots mark every high that MVRV later more than halved from, which includes each cycle's peak, and the note dates the last reading above 3.5, the level long read as a cycle top. The data are from Coin Metrics' free community API and start in 2011, once enough coins had changed hands for realised value to mean something.

**Practical takeaway:** MVRV shows how stretched the price is against the market's own cost basis, which a price chart alone cannot. Readings below 1 have coincided with the deepest points of each bear market — 2011, 2015, late 2018 and 2022 — and with the March 2020 crash, when selling came from holders capitulating at a loss. At the top of the range the record is less tidy: each cycle's peak has come lower, from 7.7 in 2011 to 4.0 in early 2021 and under 3 since, and the price tops of November 2021 and October 2025 came with MVRV below 3, never reaching the 3.5 once read as the top. One explanation is that more coins now sit with long-term holders and funds; either way, the gauge is best read against its own recent peaks rather than a fixed line.
""",
