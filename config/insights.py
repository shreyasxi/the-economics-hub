...
""",
    ...
    "commodities_breadth": """\
**How to read this chart:** The share of 13 major commodities — crude oil (Brent and WTI), natural gas, gold, silver, platinum, copper, wheat, corn, soybeans, coffee, sugar and cocoa — trading above their own 200-day average, measured daily over three years. Each commodity is judged only against its own history, so the measure is not distorted by the fact that a barrel of oil and an ounce of gold are priced on completely different scales. Above the 50% line, most of the complex is in an uptrend; below it, most is not. The names listed on the chart say exactly which are which today.

**Practical takeaway:** A headline commodity index can be dragged up by one squeezed market while everything else falls, and breadth is what separates the two cases. A broad advance — most members rising together — points to a genuine global demand impulse and usually carries an inflation signal that central banks respond to. A narrow advance points to a supply shock in a single market, which is louder in the headlines than it is in the inflation data and tends to reverse once supply recovers. The most informative moments are the disagreements: an index making new highs while breadth falls is a rally resting on fewer and fewer markets, historically a fragile one. Persistent readings below 25% or above 75% mark the extremes from which the complex has tended to mean-revert.
""",
