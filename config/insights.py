...
**How to read this chart:** Central government spending in each month of the financial year, in ₹ lakh crore: capital expenditure in blue and revenue expenditure in red, derived from the year-to-date figures in the Controller General of Accounts' monthly accounts. The black line (right axis) is capital expenditure as a share of that month's total spending, and the dashed line marks a 25% reference level, not an official target. March usually shows a jump in revenue spending as departments close the year's accounts.

**Practical takeaway:** A higher capital share means more of each rupee goes into assets such as roads, railways and defence equipment rather than salaries, interest and subsidies, which supports growth beyond the current year. Monthly capex is lumpy, so the pattern across the year matters more than any single month: weak spending in the first half followed by a rush in the final quarter points to execution delays. Read it with the Fiscal Consolidation Tracker, which shows how close full-year capex came to the Budget Estimate.
""",

    "india_fiscal_tracker": """\
**How to read this chart:** Two year-to-date paths through the financial year (April to March), in ₹ lakh crore, each against the previous year's path (grey dashed line). Left: cumulative capital expenditure, with the Budget Estimate as a dashed line and the latest total labelled with its share of that estimate. Right: the cumulative fiscal deficit. The Controller General of Accounts cautions that the deficit within a year is shaped by the timing of receipts and spending, so the year-end figure matters more than any single month.

**Practical takeaway:** Capex running ahead of last year's path shows the investment programme being delivered on schedule; a large shortfall against the Budget Estimate late in the year shows underspending, which can happen when revenue disappoints and spending is held back to protect the deficit target. A deficit path well above last year's without a matching rise in capex points to weaker receipts or higher current spending rather than more investment.
""",
