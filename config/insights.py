...
""",
    ...
    "macro_rate_cycle": """\
**How to read this chart:** Each bar counts central banks, not rate moves. Of the 38 central banks in the BIS policy rate database, which covers the Fed, the ECB, the Bank of Japan, the RBI and most large emerging-market central banks, it shows how many ended the month with a higher policy rate than a month earlier (red, above the line) and how many with a lower one (blue, below). A bank that moved twice in a month counts once, and the euro area counts once, as the ECB. The BIS publishes about a week late, so for the Fed, ECB, Bank of England, Bank of Japan and RBI any move since its last day is read from the bank's own announcement; the latest month's bar stays faded until the month is over and every bank has reported. China is counted by the one-year loan prime rate, the rate the BIS records; the strip above shows the PBoC's 7-day reverse repo rate, which the loan prime rate follows.

**Practical takeaway:** Central banks rarely move alone. The bars come in waves because they face the same oil prices, the same dollar and the same swings in global demand. The largest easing waves came with recessions: 28 banks cut in December 2008 and 30 in March 2020. The sharpest tightening was in 2022, when 28 raised rates in September alone and hikes outnumbered cuts 225 to 12 over the year. 2024 and 2025 were an easing wave, with 111 and 112 cuts against 11 and 7 hikes. From May 2026 the balance turned: hikes have matched or outnumbered cuts every month since, and the Fed, ECB and Bank of Japan all raised rates in September. A synchronised turn matters beyond any one economy, because it lifts borrowing costs everywhere at once, including the cost of dollar funding for emerging markets such as India. The count measures breadth, not size: a quarter-point hike by a small central bank weighs the same as one by the Fed.
""",
