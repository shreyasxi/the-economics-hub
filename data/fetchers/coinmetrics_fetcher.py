...
Bitcoin's MVRV ratio and price, daily since July 2010, from Coin Metrics'
free community API (no key; rate-limited to 10 requests per 6 seconds).
...
community tier here carries MVRV, price, hash rate, active addresses and
exchange flows; realised cap itself, NVT and supply-age metrics are paid.
...
_METRICS = {"CapMVRVCur": "mvrv", "PriceUSD": "price"}


def fetch_btc_mvrv(max_age_days: int = 7) -> pd.DataFrame:
    """
    Daily Bitcoin 'mvrv' and 'price' (USD), indexed by date.
    ...
    if (df <= 0).any().any():
        raise ValueError("Coin Metrics returned a non-positive MVRV or price")
